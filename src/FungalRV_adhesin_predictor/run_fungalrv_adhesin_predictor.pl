...
close FH;
$file = join('', @arr);
# Katelyn Nguyen modified 2025-06-05 to accept a path to a file as input
use File::Basename;
my $basename = basename($filename);
open(FI,">header_$basename") or die "failed to open file because of $!";
...
open(FI,"<header_$basename") or die "failed to open because $!\n";
@head=<FI>;
