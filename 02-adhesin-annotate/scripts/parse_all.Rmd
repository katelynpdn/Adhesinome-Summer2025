...
filtered_hmm_df <- hmmscan_df %>% 
  ...
  separate(query_name, into = c("tmp", "ID", "Name"), sep = "\\|") %>%
  group_by(ID) %>%
...
ST.window %>% 
  ...
  mutate(residue = factor(residue, levels = c("ST", "S", "T"), labels = c("Ser/Thr", "Ser", "Thr")),
         `Adhesin` = ifelse(failed_tests == "", "Yes", "No")) %>% 
...
# Split into Three (passing three conditions), Two (passing two conditions), and One (passing at least one condition)
