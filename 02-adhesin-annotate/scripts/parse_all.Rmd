...
params <- list(args = "../results/aurisOther ../../01-three-part-adhesin-test/results/proteinTable.csv")
...
output_table = paste0(args[1], "/annotations_table.csv")
...
all_hmm_df <- merge(filtered_hmm_df, part_one_df[, c("Protein ID", "FungalRV Score")], by = "Protein ID", all.y = TRUE)
...
ST.window %>% 
  ...
  facet_grid(`Adhesin` ~ residue, scales = "free_y", labeller = "label_both", switch = "y") + + theme_minimal(base_size = 12) +
  theme(
...
```{r}
# # Drop any NA name columns (if you ran the annotation programs on a subset of the proteome)
# all_hmm_df <- all_hmm_df[!is.na(all_hmm_df$"Name"), ]
# rownames(all_hmm_df) <- NULL
all_hmm_df <- all_hmm_df %>% relocate("FungalRV Score", .after = "Name")
write.csv(all_hmm_df, file = output_table)
print(paste0("Annotated proteins table saved to ", output_table))
```